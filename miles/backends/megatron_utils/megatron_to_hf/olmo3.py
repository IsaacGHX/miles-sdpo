...
def convert_olmo3_to_hf(args, name, param):
    """Megatron -> HF weight name/shape conversion for dense Olmo3
    (allenai/Olmo-3-7B-Instruct). Used to sync trained policy weights to the
    SGLang rollout engine each step.

    Olmo3 is PURE POST-NORM with full-dim QK-norm (see miles_plugins.models.olmo3):
      * NO fused input / pre-mlp layernorm (linear_qkv / linear_fc1 are plain);
      * post_self_attn_layernorm.weight  -> HF post_attention_layernorm.weight;
      * post_mlp_layernorm.weight        -> HF post_feedforward_layernorm.weight;
      * QK-norm over the full q/k dim     -> HF self_attn.q_norm / k_norm.
    """
    ...
    if match:
        ...
        # post-norms (Olmo3 is post-norm, no fused pre-norms)
