#!/bin/bash
# SDPO/RLSD/GRPO ablation matrix -- Olmo-3-7B-Instruct on SciKnowEval (MCQ,
# single-step, non-agentic), single 8x A100 (80GB) node, COLOCATE variant.
#
# Sibling of ../run-olmo3-7B-sdpo-sci-colocate.sh: SAME rollout/eval/skill
...
#
# Olmo-3-7B-Instruct is NON-THINKING (matches run-olmo3-7B-sdpo-sci-colocate.sh's
...
# ../run-olmo3-7B-sdpo-sci-colocate.sh's own H200 numbers: MAX_TOKENS_PER_GPU
# default 24576, arms e/f (heaviest -- extra skill forward/backward) drop to
# 16384. --sglang-mem-fraction-static default 0.9 (matching the sci-colocate
# script's own Olmo-3-7B number). TP=1 kept (Olmo-3-7B dense, ~14GB bf16
...
#
# Real single-case verification (2026-08-08, on an 8xH200 node): GRPO+arm e
# ran end-to-end for 8 train steps with no crash -- sdpo_skill_kd_loss/
# skill_kl_correct/skill_kl_pitfall all nonzero and stable, confirming
# self-success + pitfall-condense both actually fire under plain GRPO
# (--sdpo-logprob-mode sampled without --sdpo-rlsd). Peak VRAM observed:
# ~94.5GB/143.7GB per GPU (steady across 8 steps, not still climbing) at the
# OLD A100-scoped MAX_TOKENS_PER_GPU=4096/mem-fraction=0.6 for arm e -- so the
# new 16384/0.9 H200 numbers below are NOT yet re-verified against a fresh
# OOM check; retune on the first real OOM under these bumped defaults.
#
...
#   SDPO_ABLATION_SGLANG_MEM_FRACTION (default 0.9 -- H200 sizing, see header)
...
#     bash examples/SDPO/ablation/run-olmo3-7B-sdpo-ablation-sci.sh
set -exf
...
SGLANG_ARGS=(
   ...
   --sglang-mem-fraction-static "${SDPO_ABLATION_SGLANG_MEM_FRACTION:-0.9}"
