...
# --pause-generation-mode in_place: see run-qwen3-4B-sdpo-math-rl-colocate.sh's
# own MISC_ARGS comment -- the retract-mode weight-update hang reproduced on
# THIS script too (arm1, plain GRPO), confirming it is a model-independent
# infra bug, not Qwen3-4B-specific. in_place avoids the suspected wedge path
# (retract's request re-queue-on-pause / re-add-on-resume).
MISC_ARGS=(
