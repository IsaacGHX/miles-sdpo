    ...
    bash -euxc '
        ...
        case "${SDPO_MODEL}" in
            ...
            qwen25-7b-math-rl-ablation)
                ...
                # of Qwen3-4B. Built to isolate whether three back-to-back
                # GPU hangs seen on the Qwen3-4B math-rl ablation (all at the
                # weight-update -> next-rollout-batch transition, across
                # totally different SDPO configs) are specific to Qwen3-4B --
                # see the module docstring in
                # run-qwen2.5-7B-sdpo-math-rl-colocate.sh. Requires
                # $SDPO_ABLATION_ARM.
                MODEL_DIR=Qwen2.5-7B-Instruct
                HF_REPO=Qwen/Qwen2.5-7B-Instruct
                MODEL_SH=scripts/models/qwen2.5-7B.sh
