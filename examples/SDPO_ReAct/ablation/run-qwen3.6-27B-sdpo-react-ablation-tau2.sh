#!/bin/bash
# SDPO/RLSD/GRPO ablation matrix -- Qwen3.6-27B on Tau2-bench (retail/airline/
...
#
# Sibling of ../ablation/run-qwen3.5-9B-sdpo-react-ablation-tau2.sh -- copied
# verbatim and re-pointed at Qwen3.6-27B: SAME megatron spec as Qwen3.5
# (qwen3_5, confirmed via diffing scripts/models/qwen3.5-27B.sh against
# scripts/models/qwen3.6-27B.sh -- architecturally identical, only a header
# comment differs), so the SAME --tito-model qwen35 / TOOL_GRAMMAR=qwen3_coder
# tokenizer plumbing applies unchanged; this is a weights swap + parallelism
# retune for the larger dense model, not a new domain integration.
#
# TP=4 (not 9B's TP=2): 27B dense is ~3x the 9B's params; matches the 27B
# DENSE branch in run-qwen3.5-27B-35BA3B-sdpo-react-native.sh (native math/
# code/search script, same architecture family) which already uses TP=4 +
# --sequence-parallel for the SAME reason (vocab-parallel cross-entropy +
# activation sharding). --max-tokens-per-gpu lowered further from the 9B
# script's 6144/3072 (arm a/e-f) to 4096/2048 as a conservative starting
# point for the larger model's activation footprint -- untested on real
# hardware yet, adjust via SDPO_ABLATION_MAX_TOKENS_PER_GPU if this OOMs or
# leaves headroom.
#
# --tito-model qwen35 (NOT qwen3 -- Qwen3.5-family models, Qwen3.6 included,
# need their own TITO tokenizer type, confirmed present in miles/utils/
# chat_template_utils/tito_tokenizer.py's TITOTokenizerType.QWEN35).
#
# A100-80G retune + path parameterization: see the mathcodesearch sibling
# script's header for the full rationale.
#
# --log-probs-chunk-size 4096 (see the 9B sibling script's header for the
# full OOM writeup -- tau2 conversations run far longer than the other two
# ablation domains, so this chunking fix is domain-driven, not model-size-
# driven, and carries over unchanged).
#
# usage:
#   SDPO_ABLATION_ALGO=grpo SDPO_ABLATION_ARM=a \
#     bash examples/SDPO_ReAct/ablation/run-qwen3.6-27B-sdpo-react-ablation-tau2.sh
set -exf
...
# TP=4 (not the 9B sibling's TP=2) -- see module docstring's parallelism note.
...
MODEL_ARG_SH=scripts/models/qwen3.6-27B.sh
TOOL_GRAMMAR=qwen3_coder
# Conservative starting budget for the larger dense model (vs the 9B
# sibling's 6144/3072) -- untested on real hardware yet, adjust via
# SDPO_ABLATION_MAX_TOKENS_PER_GPU if this OOMs or leaves headroom.
...
PERF_ARGS=(
   --tensor-model-parallel-size "${SDPO_REACT_TP:-4}"
   ...
   # TP=4 (see module docstring's parallelism note) needs sequence-parallel
   # to shard the activations/LayerNorm too.
   --sequence-parallel
)
...
# dump on the 9B sibling script: 83% of a 257-episode sample had ZERO tool/
...
# missing flag is the primary cause of eval success (33%/56%/4% retail/
# airline/telecom) landing far below the Qwen tech report's 79.1/79.9.
#
