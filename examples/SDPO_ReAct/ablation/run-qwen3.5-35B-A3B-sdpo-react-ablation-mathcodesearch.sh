#!/bin/bash
# SDPO/RLSD/GRPO ablation matrix -- Qwen3.5-35B-A3B MoE (256 experts, top-8,
# ~3B active) on Math+Code+Search (native multi-turn <tool_call> rollout,
# SDPO_REACT_DOMAIN=multitask), single 8x A100 (80GB) node, COLOCATE variant.
#
...
# MoE NOTES header (read that script's header comment for the full R3/
# expert-parallel rationale -- not re-derived here): experts sharded 8-way
# across the node (--expert-model-parallel-size 8, --expert-tensor-parallel-
# size 1), --sglang-ep-size 8 for the rollout engine, --use-rollout-routing-
# replay so the training forward replays SGLang's own per-token expert
# choices instead of re-routing (R3, arXiv:2510.11370 -- fixes the on-policy
# routing mismatch between SGLang's and Megatron's MoE kernels), and
# --moe-token-dispatcher-type flex (the variable-length dispatcher this
# rollout's packed sequences need). --moe-aux-loss-coeff 0 (no load-balance
# loss in RL, would fight the policy gradient) comes from the model-arg
# script (scripts/models/qwen3.5-35B-A3B.sh), not re-set here.
#
...
# which explicitly sizes for 141GB): EP=8/TP=1 stays UNCHANGED from the H200
# script -- expert-parallel sharding is orthogonal to per-GPU VRAM, each of
# the 8 GPUs still holds only 1/8 of the 256 experts regardless of card
# size. What DOES need retuning is per-GPU activation memory: MAX_TOKENS_PER_
# GPU default 4096 (vs the H200 script's 8192), arms e/f (heaviest -- extra
# skill forward/backward) drop further to 2048. --sglang-mem-fraction-static
# default 0.5 (vs H200's 0.7). Checkpoint save uses the SAME
# monkey_patch_dist_checkpointing_merge() fix the H200 script's own header
# documents (already core infra, miles/utils/reloadable_process_group.py --
# no new code needed here) -- the underlying dp_reshardable-vs-fully_
# reshardable OOM tradeoff that fix resolves is VRAM-size-independent.
...
# retune further on the first real OOM, same iterative process every existing
# script's own memory-tuning comments describe.
#
...
#   SDPO_ABLATION_MAX_TOKENS_PER_GPU  (default 4096, 2048 for arms e/f)
#   SDPO_ABLATION_SGLANG_MEM_FRACTION (default 0.5)
#   SDPO_REACT_EP_SIZE                (default 8) expert-parallel size
#
# usage:
#   SDPO_ABLATION_ALGO=rlsd SDPO_ABLATION_ARM=d \
#     bash examples/SDPO_ReAct/ablation/run-qwen3.5-35B-A3B-sdpo-react-ablation-mathcodesearch.sh
set -exf
...
# MoE: keep TP=1, shard the 256 experts with EXPERT parallelism instead (EP=8
# across the node) -- see the module docstring's MoE-wiring note. NO
# --dist-ckpt-optim-fully-reshardable here (unlike the dense 4B/9B scripts):
# the H200 native script's own header explains why fully_reshardable's full-
# gather-per-rank save OOMs even on 141GB for this model -- dp_reshardable
# (the default, no flag) + the dist_checkpointing merge-truncation patch
# (already core infra) is the correct combo, same as that script settled on.
...
echo "MODEL: ${MODEL_NAME} | tool-parser=${TOOL_PARSER} | tool-grammar=${TOOL_GRAMMAR} | ep=${SDPO_REACT_EP_SIZE} | max_tokens_per_gpu=${MAX_TOKENS_PER_GPU}"
...
PERF_ARGS=(
   ...
   # Chunk the log-probs logits computation to avoid OOM at TP=1 (the vocab-
   # parallel cross-entropy buffer [tokens, vocab=248320] is NOT sharded) --
   # same flag/rationale as the H200 native script's own MoE branch.
...
# One 8-GPU EP engine for the MoE (matches the H200 native script's own
# reference: the MoE inference wants all experts co-resident, so the engine
# spans the node) + --sglang-ep-size for R3-aware rollout.
SGLANG_ARGS=(
...
# R3 (rollout routing replay) ON by default for this MoE model -- see the
# module docstring's MoE-wiring note. Train-side flag also drives the
# rollout payload's return_routed_experts (generate_endpoint_utils reads the
# same arg), so no separate sglang flag beyond --sglang-ep-size is needed.
R3_ARGS=()
...
MISC_ARGS=(
   ...
   # Widen the NCCL/process-group watchdog window -- the MoE's distributed
   # checkpoint save is slow enough that ranks can sit in the save collective
   # past the 10-min default, same rationale as the H200 native script.
   --distributed-timeout-minutes "${SDPO_REACT_DIST_TIMEOUT_MIN:-60}"
   # flex token dispatcher: the variable-length dispatcher this rollout's
   # packed sequences need (matches scripts/models/qwen3.5-35B-A3B.sh's own
   # reference launcher).
